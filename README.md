...
- `Start-AtlasDataServer-OffloadDisabled.ps1` — launches
  ...
  250 ms for the lifetime of the process. Also shows the control window
  below.
...
While ADS is open, a small **ADS Offload Disabler** window sits beside it
(with its own taskbar button) showing **Next offload: DISABLED**. Click
**Allow offload** to let an offload happen: the "Disable Next Offload" box
in ADS is unticked and left alone, and the window shows **Next offload:
ALLOWED** with a warning icon. Click **Disable offload** to go back.

Every ADS launch starts with offloads disabled. Closing the window only
minimises it; it closes by itself when ADS closes.
